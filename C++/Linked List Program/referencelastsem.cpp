...
void entryScreen()
{
    ...
    cout << "\t\t\t\t   The Date and Time today is: ";
    printf("%s", ctime(&timeToday)); ///date and time syntax
    cout << "\t\t\t   ====================================================================\n";
...
{   /////////////////////////////////for inputting new queries///////////////////////
    ...
    for (int i = 0; i < ctr; i++)
    {
        ...
        getline(cin, orderArr[i].contactNumber);
        table[i][0] = orderArr[i].contactNumber;

        cout << "\tEnter the price: ";
        cin >> orderArr[i].price;
        table[i][1] = orderArr[i].price;
        cin.ignore();

        cout << "\tInput Customer Name: ";
        getline(cin, orderArr[i].customerName);
        (*q).enQueue(orderArr[i].customerName);
        table[i][3] = orderArr[i].customerName;

        cout << "\tDelivery Type: (COD, Card, Bank Transfer...)";
        getline(cin, orderArr[i].deliveryType);
        table[i][2] = orderArr[i].deliveryType;

        cout << "\tEnter Customer Address: ";
        getline(cin, orderArr[i].address);
...
void summary()
{
    ...
    for (int i = 0; i < ctr; i++)
    {
        cout << "Enter Status (numbers only) for " << orderArr[i].customerName << ": ";
        cin >> status[i];
    }
    system("pause");
}
...
{
    system("CLS");
    cout << "\t\t\t\t  -- Here is the registered delivery staff available for today:  --" << endl
         << endl;
